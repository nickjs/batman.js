...
`Batman.SetIndex` is a grouped collection of items derived from a [`Batman.Set`]() filled with [`Batman.Object`]()s. It extends `Batman.Object` and [`Batman.Enumerable`](), so it inherits methods from them, too. In short, a `SetIndex` tracks its base `Set` and contains "buckets" of items from that `Set`, grouped by the provided key.
...
weapons = new Batman.Set(batarang, grappleGun, fists)
weaponsByType = new Batman.SetIndex(set, 'type')

weaponsByType.get('ranged.length') # => 2
weaponsByType.get('melee.length')  # => 1
weaponsByType.get('ranged').map (weapon) -> weapon.get('name')  # => ["Batarang", "GrappleGun"]
weaponsByType.toArray()                                         # => ["ranged", "melee"]
```
...
`Batman.UniqueSetIndex` extends `SetIndex` but adds a new consideration: its index contains _first matching item_ for each value of the `key` (rather than all matching items).
...
weapons = new Batman.Set(batarang, grappleGun, fists)
weaponsByUniqueType = new Batman.UniqueSetIndex(set, 'type')
weaponsByUniqueType.get('ranged.name') # => "Batarang"
```
...
# Batman.SetSort
# Batman.SetUnion
# Batman.SetIntersection
# Batman.SetComplement
