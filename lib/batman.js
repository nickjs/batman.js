...
(function() {
  ...
  Batman.Association = (function() {
    ...
    function Association(model, label, options) {
      var defaultOptions, encoder, getAccessor, self, _base;
      this.model = model;
      this.label = label;
      if (options == null) {
        options = {};
...
(function() {
  ...
  Batman.DOM.AttachmentBinding = (function(_super) {

    __extends(AttachmentBinding, _super);

    AttachmentBinding.prototype.isTwoWay = false;

    function AttachmentBinding(node, className, key, context, parentRenderer, invert) {
      this.invert = invert != null ? invert : false;
      this.placeholderNode = document.createComment("detached node " + (this.get('_batmanID')));
      AttachmentBinding.__super__.constructor.apply(this, arguments);
      if (this.placeholderNode != null) {
        Batman.DOM.trackBinding(this, this.placeholderNode);
      }
    }

    AttachmentBinding.prototype.dataChange = function(value) {
      var parentNode;
      if (!!value === !this.invert) {
        parentNode = this.placeholderNode.parentNode;
        Batman.DOM.willInsertNode(this.node);
        parentNode.insertBefore(this.node, this.placeholderNode);
        Batman.DOM.didInsertNode(this.node);
        return parentNode.remove(this.placeholderNode);
      } else {
        parentNode = this.node.parentNode;
        parentNode.insertBefore(this.placeholderNode, this.node);
        Batman.DOM.willRemoveNode(this.node);
        parentNode.removeNode(this.node);
        return Batman.DOM.didRemoveNode(this.node);
      }
    };
