(function() {
  ...
  BatmanObject.classAccessor('className', function() {
    if (Batman.config.minificationErrors) {
      developer.error("Please give the class " + ($functionName(this)) + " a className property for minification saftey!");
    }
  ...
  Batman.Controller = (function(_super) {
    ...
    Controller.classAccessor('className', function() {
      if (this.className != null) {
        return this.className;
      } else if (this.prototype.routingKey != null) {
        return helpers.camelize(this.prototype.routingKey);
      } else {
        if (Batman.config.minificationErrors) {
          developer.error("Please define className on the class or routingKey on the prototype of " + ($functionName(this)) + " in order for your controller to be minification safe.");
        }
        return $functionName(this).replace('Controller', '');
      }
    });
    ...
    Controller.prototype.dispatch = function(action, params) {
      ...
      params.controller || (params.controller = this.constructor.get('className'));
    ...
    Controller.prototype.executeAction = function(action, params) {
      ...
      developer.assert(this[action], "Error! Controller action " + (this.constructor.get('className')) + "." + action + " couldn't be found!");
    ...
    Controller.prototype.redirect = function(url) {
      ...
      if (frame) {
        if (frame.actionTaken) {
          developer.warn("Warning! Trying to redirect but an action has already be taken during " + (this.constructor.get('className')) + "." + (frame.action || this.get('action')) + "}");
        }
    ...
    Controller.prototype.render = function(options) {
      ...
      if (frame && frame.actionTaken && this._renderedYields[options.into]) {
        developer.warn("Warning! Trying to render but an action has already be taken during " + (this.get('className')) + "." + action + " on yield " + options.into);
      }
      if (frame != null) {
        frame.actionTaken = true;
      }
      ...
      if (!options.view) {
        options.viewClass || (options.viewClass = ((_ref2 = Batman.currentApp) != null ? _ref2[helpers.camelize("" + (this.constructor.get('className')) + "_" + action + "_view")] : void 0) || Batman.View);
        options.context || (options.context = this.get('_renderContext'));
        options.source || (options.source = helpers.underscore(this.constructor.get('className') + '/' + action));
  ...
  Batman.Model = (function(_super) {
    ...
    Model.classAccessor('className', function() {
      if (this.className != null) {
        return this.className;
      } else if (this.prototype.storageKey != null) {
        return helpers.camelize(helpers.singularize(this.prototype.storageKey));
      } else {
        if (Batman.config.minificationErrors) {
