    ...
    SimpleSet.prototype.addAndRemove = function(itemsToAdd, itemsToRemove) {
      ...
      if (this.fire) {
        ...
        if (itemsAdded.length > 0) {
          this.fire.apply(this, ['itemsWereAdded'].concat(__slice.call(itemsAdded)));
        }
        if (itemsRemoved.length > 0) {
          this.fire.apply(this, ['itemsWereRemoved'].concat(__slice.call(itemsRemoved)));
        }
      }
      return {
        added: itemsAdded,
...
(function() {
  ...
  Batman.Renderer = (function(_super) {
    var bindingSortOrder, bindingSortPositions, k, name, pos, _i, _j, _len, _len1, _ref;
    ...
    Renderer.prototype.start = function() {
      ...
      return this.parseTree(this.node);
    };

    Renderer.prototype.resume = function() {
      this.startTime = new Date;
      return this.parseTree(this.resumeNode);
    };
    ...
    Renderer.prototype.parseNode = function(node) {
      var attr, attrIndex, attribute, binding, bindingDefinition, bindings, oldContext, reader, skipChildren, value, _k, _l, _len2, _len3, _ref1, _ref2, _ref3, _ref4,
        _this = this;
...
(function() {
  ...
  Batman.SetIndex = (function(_super) {
    ...
    function SetIndex(base, key) {
      ...
      if (this.base.isEventEmitter) {
        ...
        this._setObserver.on('itemsWereAdded', function() {
          var items, theseItems, _ref;

          items = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
          _ref = _this.itemsByKey(items);
          for (key in _ref) {
            if (!__hasProp.call(_ref, key)) continue;
            theseItems = _ref[key];
            _this._addToKey(key, theseItems);
          }
        });
        this._setObserver.on('itemsWereRemoved', function() {
          ...
          for (key in _ref) {
            if (!__hasProp.call(_ref, key)) continue;
            theseItems = _ref[key];
            _this._removeFromKey(key, theseItems);
          }
        });
      }
      _ref = this.itemsByKey(this.base._storage);
      for (key in _ref) {
        if (!__hasProp.call(_ref, key)) continue;
        items = _ref[key];
    ...
    SetIndex.prototype.itemsByKey = function(items) {
      var byKey, item, key, _i, _len;

      byKey = {};
      for (_i = 0, _len = items.length; _i < _len; _i++) {
        item = items[_i];
        key = this._keyForItem(item);
        byKey[key] || (byKey[key] = []);
        byKey[key].push(item);
      }
      return byKey;
    ...
    SetIndex.prototype.observerForItemAndKey = function(item, key) {
      ...
      return function(newKeyValue, oldKeyValue) {
        console.log(item, key);
        _this._removeFromKey(oldKeyValue, item);
        return _this._addToKey(newKeyValue, item);
      };
    };
    ...
    SetIndex.prototype._addToKey = function() {
      var items, key;

      key = arguments[0], items = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      return this._resultSetForKey(key).add(items);
    };

    SetIndex.prototype._removeFromKey = function() {
      var items, key;

      key = arguments[0], items = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      return this._resultSetForKey(key).remove(items);
    };
...
(function() {
  ...
  Batman.PolymorphicAssociationSetIndex = (function(_super) {
    ...
    PolymorphicAssociationSetIndex.prototype._addToKey = function(key, items) {
      ...
      return PolymorphicAssociationSetIndex.__super__._addToKey.call(this, key, filteredItems);
    };

    PolymorphicAssociationSetIndex.prototype._removeFromKey = function(key, items) {
      var filteredItems, item;
      ...
      return PolymorphicAssociationSetIndex.__super__._removeFromKey.call(this, key, filteredItems);
    };
...
(function() {
  ...
  Batman.UniqueSetIndex = (function(_super) {
    ...
    UniqueSetIndex.prototype._addToKey = function(key, items) {
      this._resultSetForKey(key).add(items);
      if (!this._uniqueIndex.hasKey(key)) {
        return this._uniqueIndex.set(key, items[items.length - 1]);
      }
    };

    UniqueSetIndex.prototype._removeFromKey = function(key, items) {
      var resultSet;

      resultSet = this._resultSetForKey(key);
      UniqueSetIndex.__super__._removeFromKey.apply(this, arguments);
  ...
  Batman.View = (function(_super) {
    ...
    View.prototype.applyYields = function() {
      return this.get('yields').forEach(function(name, nodes) {
        var action, node, yieldObject, _i, _len, _ref1;
