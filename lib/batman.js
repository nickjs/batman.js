...
(function() {
  ...
  Batman.SimpleSet = (function() {
    ...
    SimpleSet.prototype.insert = function() {
      var addedItems, index, item, itemsWithIndexes, _i, _len, _ref;
      itemsWithIndexes = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      addedItems = [];
      for (_i = 0, _len = itemsWithIndexes.length; _i < _len; _i++) {
        _ref = itemsWithIndexes[_i], item = _ref[0], index = _ref[1];
        if (!(!~this._indexOfItem(item))) {
          continue;
        }
        this._storage.splice(index, 0, item);
        addedItems.push([item, index]);
      }
      this.length = this._storage.length;
      return addedItems;
    };
    ...
    SimpleSet.prototype.addAndRemove = function(itemsToAdd, itemsToRemove) {
      ...
      addedItems = this.add(itemsToAdd || []);
      removedItems = this.remove(itemsToRemove || []);
      return {
        added: addedItems,
...
(function() {
  ...
  Batman.Property = (function(_super) {
    ...
    Property.prototype.sourceChangeHandler = function() {
      var _this = this;
      if (this._sourceChangeHandler == null) {
        this._sourceChangeHandler = this._handleSourceChange.bind(this);
      }
...
(function() {
  ...
  Batman.Set = (function(_super) {
    ...
    Set.prototype.insert = Set.mutation(function() {
      var addedItems;
      addedItems = Batman.SimpleSet.prototype.insert.apply(this, arguments);
      if (addedItems.length) {
        this.fire('itemsWereAddedAtPositions', addedItems);
      }
      return addedItems;
    });

    Set.prototype.remove = Set.mutation(function() {
      var removedItems;
      removedItems = Batman.SimpleSet.prototype.remove.apply(this, arguments);
    ...
    Set.prototype.addAndRemove = Set.mutation(function(itemsToAdd, itemsToRemove) {
      var addedItems, removedItems;
      addedItems = Batman.SimpleSet.prototype.add.apply(this, itemsToAdd || []);
      removedItems = Batman.SimpleSet.prototype.remove.apply(this, itemsToRemove || []);
      if (addedItems.length) {
        this.fire('itemsWereAdded', addedItems);
      }
      if (removedItems.length) {
  ...
  Batman.SetProxy = (function(_super) {
    ...
    function SetProxy(base) {
      ...
      if (this.base.isCollectionEventEmitter) {
        ...
        this.base.on('itemsWereAdded', function(items) {
          _this.set('length', _this.base.length);
          return _this.fire('itemsWereAdded', items);
        });
        this.base.on('itemsWereRemoved', function(items) {
          _this.set('length', _this.base.length);
          return _this.fire('itemsWereRemoved', items);
        });
      }
    }
    ...
    _ref = ['add', 'insert', 'remove', 'addAndRemove', 'at', 'find', 'clear', 'has', 'merge', 'toArray', 'isEmpty', 'indexedBy', 'indexedByUnique', 'sortedBy'];
    _fn = function(k) {
...
(function() {
  var longestNondecreasingSubsequence,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  longestNondecreasingSubsequence = function(values, compare) {
    var M, currIndex, end, index, lastIndex, newIndex, predecessors, start, subsequence, subsequenceLength, val, _i, _len, _ref;
    subsequenceLength = 0;
    M = [];
    predecessors = [];
    newIndex = 0;
    for (index = _i = 0, _len = values.length; _i < _len; index = ++_i) {
      val = values[index];
      start = 1;
      end = subsequenceLength + 1;
      lastIndex = (subsequenceLength >> 1) + 1;
      while (lastIndex >= 1) {
        if (start === end) {
          break;
        }
        newIndex = lastIndex;
        if ((_ref = compare(values[M[lastIndex - 1] - 1], val)) === (-1) || _ref === 0) {
          start = lastIndex + 1;
        } else {
          end = lastIndex;
        }
      ...
      if (compare(values[M[newIndex - 1] - 1], val) === 1) {
        newIndex = 0;
      }
      predecessors[index] = M[newIndex - 1];
      if (newIndex === subsequenceLength || val < values[M[newIndex] - 1]) {
        M[newIndex] = index + 1;
        subsequenceLength = Math.max(subsequenceLength, newIndex + 1);
      }
    }
    currIndex = M[subsequenceLength - 1];
    subsequence = [];
    while (currIndex != null) {
      subsequence.unshift(values[currIndex - 1]);
      currIndex = predecessors[currIndex - 1];
    }
    return subsequence;
  };

  window.binarySearch = function(array, item, compare, exactMatch) {
    var direction, end, index, start;
    if (exactMatch == null) {
      exactMatch = true;
    }
    start = 0;
  ...
  Batman.SetSort = (function(_super) {
    ...
    function SetSort(base, key, order) {
      ...
      if (this.base.isObservable) {
        ...
        this._setObserver.on('itemsWereAdded', function(items) {
          var addedItems, index, item, oldStorage, _i, _len;
          if (false && items.length > Math.log(_this._storage.length) * 5) {
            return _this._reIndex();
          } else {
            oldStorage = _this._storage;
            addedItems = [];
            for (_i = 0, _len = items.length; _i < _len; _i++) {
              item = items[_i];
              index = binarySearch(_this._storage, item, _this.compareElements, false);
              _this._storage.splice(index, 0, item);
              addedItems.push([item, index]);
            }
            _this.property('_storage').fire(_this._storage, oldStorage, '_storage');
            return _this.fire('itemsWereAddedAtPositions', addedItems);
          }
        });
        this._setObserver.on('itemsWereRemoved', function(items) {
          var index, item, oldStorage, removedItems, _i, _len;
          if (false && items.length > Math.log(_this._storage.length) * 5) {
            return _this._reIndex();
          } else {
            oldStorage = _this._storage;
            ...
            _this.property('_storage').fire(_this._storage, oldStorage, '_storage');
            return _this.fire('itemsWereRemovedFromPositions', removedItems);
          }
        });
        this.startObserving();
      }
    ...
    SetSort.prototype._indexOfItem = function(item) {
      return binarySearch(this._storage, item, this.compareElements);
    };
...
(function() {
  ...
  Batman.View = (function(_super) {
    ...
    function View() {
      ...
      this.subviews.on('itemsWereAddedAtPositions', function(newSubviews) {
        var index, subview, _i, _len, _ref;
        for (_i = 0, _len = newSubviews.length; _i < _len; _i++) {
          _ref = newSubviews[_i], subview = _ref[0], index = _ref[1];
    ...
    View.prototype.insertIntoDOM = function(parentNode) {
      if (parentNode !== this.node && !parentNode.contains(this.node)) {
        if (this.nextSiblingNode) {
          return parentNode.insertBefore(this.node, this.nextSiblingNode);
        } else {
...
(function() {
  ...
  Batman.DOM.FormBinding = (function(_super) {
    ...
    FormBinding.prototype.initializeChildBindings = function() {
      var attribute, binding, errorsNode, field, index, keyPath, selectedNode, selectedNodes, selectors, _i, _len;
      keyPath = this.keyPath;
  ...
  Batman.DOM.AbstractCollectionBinding = (function(_super) {
    ...
    AbstractCollectionBinding.prototype.bindCollection = function(newCollection) {
      ...
      if (newCollection === this.collection) {
        ...
        if (this.collection instanceof Batman.SetSort && this.handleItemsAddedAtPositions && this.handleItemsRemovedFromPositions) {
          this.collection.on('itemsWereAddedAtPositions', this.handleItemsAddedAtPositions);
          this.collection.on('itemsWereRemovedFromPositions', this.handleItemsRemovedFromPositions);
          if (this.collection.length) {
            this.handleItemsAdded(this.collection.toArray());
          }
        } else if (this.collection.isCollectionEventEmitter && this.handleItemsAdded && this.handleItemsRemoved) {
    ...
    AbstractCollectionBinding.prototype.unbindCollection = function() {
      ...
      if (this.handleItemsAdded && this.handleItemsRemoved) {
        this.collection.forget('itemsWereAdded', this.handleItemsAdded);
        return this.collection.forget('itemsWereRemoved', this.handleItemsRemoved);
      } else {
...
(function() {
  ...
  Batman.IteratorView = (function(_super) {
    ...
    IteratorView.prototype.insertItem = function(item, index) {
      var iterationView, _ref1;
      ...
      if (index != null) {
        iterationView.targetIndex = index;
        iterationView.nextSiblingNode = ((_ref1 = this.subviews.at(index)) != null ? _ref1.get('node') : void 0) || this.node;
        iterationView.parentNode = this.node.parentNode;
    ...
    IteratorView.prototype.finishAppendItems = function() {
      var isInDOM, subview, _i, _j, _k, _len, _len1, _len2, _ref1, _ref2, _ref3;
      ...
      _ref2 = this.appendedViews;
      for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
        subview = _ref2[_j];
        if (subview.targetIndex != null) {
          this.subviews.insert([subview, subview.targetIndex]);
        }
      }
...
(function() {
  ...
  Batman.DOM.IteratorBinding = (function(_super) {
    ...
    function IteratorBinding(definition) {
      ...
      this.handleItemsRemovedFromPositions = __bind(this.handleItemsRemovedFromPositions, this);
      this.handleItemsAddedAtPositions = __bind(this.handleItemsAddedAtPositions, this);
      this.handleArrayChanged = __bind(this.handleArrayChanged, this);
      var _this = this;
    ...
    IteratorBinding.prototype.handleArrayChanged = function(newItems, oldItems) {
      var subsequence;
      if (!this.backingView.isDead) {
        if (newItems != null ? newItems.length : void 0) {
          if (this.collection.isSorted && newItems.length === oldItems.length) {
            return subsequence = this.longestNondecreasingSubsequence(oldItems, this.collection.compareElements);
          } else {
    ...
    IteratorBinding.prototype.handleItemsAddedAtPositions = function(addedItems) {
      var index, item, _i, _len, _ref;
      if (!this.backingView.isDead) {
        this.backingView.beginAppendItems();
        for (_i = 0, _len = addedItems.length; _i < _len; _i++) {
          _ref = addedItems[_i], item = _ref[0], index = _ref[1];
          this.backingView.insertItem(item, index);
        }
        return this.backingView.finishAppendItems();
      }
    };

    IteratorBinding.prototype.handleItemsRemovedFromPositions = function(removedItems) {
      var index, item, _i, _len, _ref, _results;
      if (!this.backingView.isDead) {
        _results = [];
        for (_i = 0, _len = removedItems.length; _i < _len; _i++) {
          _ref = removedItems[_i], item = _ref[0], index = _ref[1];
          _results.push(this.backingView.subviews.at(index).die());
        }
        return _results;
      }
    };

    IteratorBinding.prototype.handleItemsAdded = function(addedItems) {
      var item, _i, _len;
      if (!this.backingView.isDead) {
        this.backingView.beginAppendItems();
        if (addedItems) {
    ...
    IteratorBinding.prototype.handleItemsRemoved = function(removedItems) {
      var item, subview, _i, _j, _len, _len1, _ref;
      if (!this.backingView.isDead) {
        for (_i = 0, _len = removedItems.length; _i < _len; _i++) {
          item = removedItems[_i];
