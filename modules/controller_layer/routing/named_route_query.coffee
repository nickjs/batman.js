{BatmanObject, Property} = require 'foundation'


module.exports = class NamedRouteQuery extends BatmanObject
  ...
  @accessor 'routeMap', 'args', 'cardinality', 'hashValue', Property.defaultAccessor
  ...
  nextQueryForName: (key) ->
    if map = @get('routeMap').childrenByName[key]
      return new NamedRouteQuery(map, @args)
    else
  ...
  clone: (args = @args) -> new NamedRouteQuery(@routeMap, args)
