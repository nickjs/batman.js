...
PropertyEvent = require './property_event'

SOURCE_TRACKER_STACK = []
SOURCE_TRACKER_STACK_VALID = true

module.exports = class Property extends PropertyEvent
  ...
  @defaultAccessorForBase: (base) ->
    base._batman?.getFirst('defaultAccessor') or Property.defaultAccessor
  ...
  @registerSource: (obj) ->
    return unless obj.isEventEmitter || obj instanceof Property
  ...
  registerAsMutableSource: ->
    Property.registerSource(this)

  isEqual: (other) ->
    @constructor is other.constructor and @base is other.base and @key is other.key

  hashKey: ->
    @_hashKey ||= "<Property base: #{hashKeyFor(@base)}, key: \"#{hashKeyFor(@key)}\">"
  ...
  sourceChangeHandler: ->
    @_sourceChangeHandler ||= => @_handleSourceChange()
