...
Set = require './set'
{get} = require '../object_helpers'

module.exports = class SetSort extends SetProxy
  ...
  merge: (other) ->
    @base.registerAsMutableSource()
    new Set(@_storage).merge(other).sortedBy(@key, @order)
  ...
  compareElements: (a, b) =>
    valueA = if @key and a? then get(a, @key) else a
    if typeof valueA is 'function'
      valueA = valueA.call(a)

    valueA = valueA.valueOf() if valueA?

    valueB = if @key and b? then get(b, @key) else b
