{developer, functionName, mixin, extend} = require 'foundation'
...
module.exports = class Association
  ...
  constructor: (@model, @label, options = {}) ->
    ...
    @options = mixin({}, @provideDefaults(), options)

    if @options.nestUrl
      developer.error "You must persist the model #{functionName(@model)} to use the url helpers on an association" if !@model.urlNestsUnder?
      @model.urlNestsUnder helpers.underscore(@getRelatedModel().get('resourceName'))
  ...
  getRelatedModel: ->
    ...
    developer.do ->
      if !relatedModel and Batman.env isnt 'test'
        ...
        developer.warn "Related model #{className} wasn't found in namespace #{namespaceMsg}"
    relatedModel
