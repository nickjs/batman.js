URI = require './uri'
{BatmanObject, developer, typeOf} = require 'foundation'

module.exports = class Request extends BatmanObject
  @objectToFormData: (data) ->
    pairForList = (key, object, first = false) ->
      if object instanceof Batman.container.File
        return [[key, object]]

      list = switch typeOf(object)
        when 'Object'
  ...
  send: (data) -> developer.error "You must add a platform library to implement Batman.Request (for example, batman.jquery)"

  @set('pendingRequestCount', 0)
  @classAccessor 'requestIsPending', -> @get('pendingRequestCount') > 0
  @::on 'loading', ->  Request.increment('pendingRequestCount')
  @::on 'loaded', -> Request.decrement('pendingRequestCount')
