{developer, BatmanObject} = require 'foundation'
...
module.exports = class AbstractBinding extends BatmanObject
    ...
    if filters.length
      while filterString = filters.shift()
        ...
        unless filter = (@view?._batman.get('filters')?[filterName] || Batman.Filters[filterName])
          return developer.error "Unrecognized filter '#{filterName}' in key \"#{@keyPath}\"!"
  ...
  setupBackingView: (viewClass, viewOptions) ->
    ...
    return @backingView if @node and @backingView =_data(@node, 'view')
    ...
    _data(@node, 'view', @backingView) if @node
