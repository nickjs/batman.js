...
test "pathFromLocation(window.location) returns the app-relative path", ->
  location =
    pathname: @nav.normalizePath(Batman.config.pathToApp, 'foo/bar')
    search: '?page=2'
  equal @nav.pathFromLocation(location), '/foo/bar?page=2'
  equal @nav.pathFromLocation(pathname: Batman.config.pathToApp, search: ''), '/'

if Batman.PushStateNavigator.isSupported()
  test "pushState(stateObject, title, path) prefixes the path with Batman.config.pathToApp and delegates to window.history", ->
    @nav.pushState(null,'','/foo/bar')
    equal window.location.pathname, "#{Batman.config.pathToApp}/foo/bar"
