...
Watson.ensureCommitted '69ad8b18908c0171601db7198ffe5e78c73095ee', ->
  ...
  Watson.benchmark 'setsort insertion', (error, suite) ->
    throw error if error
    ...
    do ->
      ...
      a = []
      for i in [0...100]
        a.push new Clunk(foo: (letters[generator.next()] for j in [0..6]).join(''))

      set = new Batman.SetSort(ffs, 'foo')

      suite.add 'sorting on strings2', ->
        for q in a
          ffs.add q
